...
export default function TeamsTab({ players }) {
  ...
  const [dragging, setDragging] = useState(null);   // { playerId, fromTeamId }
  const [tapped, setTapped] = useState(null);        // { playerId, fromTeamId }
  ...
  function handleDragLeave(e) {
    // Only clear when leaving the card entirely, not its children
  ...
  function handleTap(teamId, playerId) {
    if (playerId !== undefined) {
      ...
      // Tapping a team card while a player is selected → move
